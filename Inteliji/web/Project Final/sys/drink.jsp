...
<div id="panel-wrapper" class="container-fluid">
    <div class="row align-items-center justify-content-md-center" data-aos="flip-left" data-aos-duration="2000">
        <nav id="panel-nav">
            <img src="<%=Utils.fullPathImg("img/drink/header-drink.jpg")%>" alt="" width="100%">
        </nav>
    </div>
    <div id="content-wrapper" class="container-fluid">
        <%try {%>
        <%
            List<SmallProductUser> dsn = (List<SmallProductUser>) request.getAttribute("listPr");
            assert dsn != null;
        %>
            ...
            <div class="content col">
                <div class="row">
                    <div class="img col"><a style="cursor: pointer" data-toggle="modal"
                                            data-target="#myModal<%=i%>"><img src="<%=Utils.fullPathImg(dsn.get(i).getImg())%>" alt="" width="200" height="200"></a>
                    </div>
                    <div class="content-text product-content col">
                        <h4 STYLE='color: yellow'><%=dsn.get(i).getName()%>
                        </h4>
                        <div style="color: white"><%=dsn.get(i).getDes()%><br>
                            Giá: <%=dsn.get(i).getPrice()%>đ
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col"></div>
                    <button onclick="return location.href='<%=Utils.fullPath("AddToCart?table=sanphamphu&id=")%><%=dsn.get(i).getId()%>'" class="button col"> BẮT ĐẦU ĐẶT HÀNG</button>
                </div>
            </div>
            ...
            <div class="content col">
                <div class="row">
                    <div class="img col"><a style="cursor: pointer" data-toggle="modal"
                            ...
                            src="<%=Utils.fullPathImg(dsn.get(i).getImg())%>" alt="" width="200" height="200"></a>
                    </div>
                    <div class="content-text product-content col">
                        <h4 STYLE='color: yellow'><%=dsn.get(i).getName()%>
                        </h4>
                ...
                <div class="modal-content" style="background: black">
                    ...
                    <div class="modal-body row">
                        <div class="img col"><a><img
                                src="<%=Utils.fullPathImg(dsn.get(j).getImg())%>" alt="" width="100%"></a>
                        </div>
                        <div class="content-text product-content col">
                            <h3 STYLE='color: yellow'><%=dsn.get(j).getName()%>
                            </h3>
                            <div style="color: white; font-size: 17px;"><%=dsn.get(j).getDes()%><br>
                                Giá: <%=dsn.get(j).getPrice()%>đ
                            </div>
                            <button onclick="return location.href='<%=Utils.fullPath("AddToCart?table=sanphamphu&id=")%><%=dsn.get(j).getId()%>'" class="button col"> BẮT ĐẦU ĐẶT HÀNG</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div id="pagination">
            ...
            <a href="<%=Utils.fullPath("ProductUserController?table=sanphamphu&type=nc&page=")+i%>" class="btn-pagi"><%=i%>
            </a>
            <%}%>
        </div>
    </div>
